...
For this dataset, the target values are ordered classes `[-1, 1, 2, 3, 4, 5]`, so the system treats the task as ordinal multiclass classification and uses weighted F1, quadratic weighted kappa, and ordinal MAE during model selection.
...
- The `-1` target value is preserved as the lowest observed ordinal tier because the dataset provides it as a valid class and no alternate business mapping is supplied.
